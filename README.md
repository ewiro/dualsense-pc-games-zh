...
- 桌面表格、移动卡片和分页；
- 白天 / 黑夜模式，选择会保存在浏览器中；
- 内置 DualSense / DualSense Edge 在线测试器，可通过 WebHID 查看按键、摇杆、扳机、触控板、六轴体感和电量，并测试自适应扳机、低强度兼容震动与 USB 高清音频触觉；
- 每周自动抓取、校验并部署最新数据。
...
USB 高清触觉需要使用数据线，并在网页列出的音频输出中选择 Wireless Controller。建议先将 Windows 手柄音频端点设为 50%，再从网页的 40% 强度开始调节；滑块完整范围为 10%–100%。Chrome 若尚未公开完整设备列表，测试器会申请一次麦克风设备权限，取得音频输出名称后立即停止媒体轨道，不会持续采集音频。测试器只在所选输出提供 4 个声道时启用触觉波形：前两个声道保持静音，后两个声道分别驱动左右执行器；细雨、砂石、脚步和引擎模式使用增强校准、全局软限幅和平滑包络，以保留细腻的左右差异并避免数字削波。蓝牙没有音频触觉通道，因此回退到已限制强度的 HID 兼容震动。HID 报告格式参考 [nondebug/dualsense](https://github.com/nondebug/dualsense) 与 [dualsense-studio](https://github.com/nikhil8182/dualsense-studio)。
...
| `npm run overrides` | 将本站人工纠正重新应用到现有数据快照 |
...
├── data/
│   ├── game-overrides.json      # 经 Issue 核验的本站人工纠正
│   ├── games.json               # 已校验的数据快照
│   └── title-translations.json  # 中英文名称映射
├── scripts/
│   ├── apply-overrides.js       # 将人工纠正应用到现有快照
...
├── haptics-audio.js            # 48 kHz 细腻触觉波形生成
├── tester.css                  # 测试器响应式样式
├── styles.css
└── light.css                    # 主题与界面增强样式
```
...
抓取器分别分页获取 DualSense 与 DualSense Edge 记录，合并去重后只保留自适应扳机或触觉反馈状态为“支持”“有限支持”“需额外调整”或“始终启用”的游戏。购买平台和商品 ID 来自各游戏页面的 `Availability` 记录；功能状态、模式与说明来自 `Input` 模板。功能说明按原文缓存中文翻译，说明中的 PCGamingWiki 站内链接和外部参考链接会被解析、保留并适配到中文文本，避免浏览器端临时请求翻译服务。经维护者核验的本站纠正保存在 `data/game-overrides.json`，在最终校验前覆盖上游字段，并通过对应 Issue 和证据链接保留来源。PCGamingWiki 暂无稳定的手柄扬声器统一字段，因此仅在页面存在明确控制器扬声器记录时标记支持，其余保留为“未知”。
